...
/**
  * @brief  Returns the number of pending messages in the receive FIFO.
...
void CAN_ClearFlag(CAN_FLAG_TypeDef CAN_Flag)
{
    ...
    {
      /*Sameer: There is a bug in this code. The mask value should be 0x0208 to
                 clear receive flags but it was set to 0x020B
                This was preventing clearing of CAN_FLAG_RQCP0 flags 
                from CAN->TSR*/  
      /*if (((uint16_t)CAN_Flag & 0x020B)!= RESET)*/
        if (((uint16_t)CAN_Flag & 0x0208)!= RESET)
        {
            /*Receive Flags*/
