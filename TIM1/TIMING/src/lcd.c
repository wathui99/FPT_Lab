...
void Lcd_Print_Char(int16_t character, uint8_t pos){
  Start_Write_Ram();
  /*!< Space character */
...
void Lcd_Print_String(char* str, uint8_t pos){
  while(*str){
    ...
    if((uint8_t)*str >= 97 && (uint8_t)*str <= 122){
      Lcd_Print_Char((uint8_t)*str-55-32,pos);
    }
    else if((uint8_t)*str >= 48 && (uint8_t)*str <= 57){
      Lcd_Print_Char((uint8_t)*str-48,pos);
    }
    else {
      Lcd_Print_Char((uint8_t)*str-55,pos);
    }
    str++;
    pos++;
  }
